...
export const metadata = {
    ...
    slug: "imageopti",
    textOnly:
        "겨울방학 이야기 글을 올리면서 문제가 있었다. 사진 용량이 너무 커서 전부 압축해도 200MB 정도가 되었고, 인터넷이 느린 환경에서는 제대로 보이지 않을 수 있었다. 이를 개선하기 위해 Next.js의 Image 컴포넌트를 적용해 이미지 로딩 최적화를 시도했다. Lazy loading을 통해 화면에 실제로 나타날 때만 이미지를 불러오게 했고, 파이썬 스크립트를 사용해 이미지의 가로·세로 크기를 자동으로 지정하도록 했다. 기존에 <img> 태그를 쓰면 이미지가 로드되면서 페이지가 깜빡이거나 깨질 수 있는데, 이런 현상을 CLS(Cumulative Layout Shift)라고 한다. 이번에는 이미지 로드 전 placeholder나 blurDataURL을 사용해, 로딩 전에도 임시 이미지가 나타나도록 구현했다. 이미지 포맷을 WebP로 바꾸면서 용량은 146MB에서 13.6MB로 90% 이상 줄었고, 로드 시간도 6.10초에서 3.01초로 절반 가까이 단축되었다. Image 컴포넌트는 이미지 크기를 알아야 하므로, MDX 파일을 파싱해 <Image> 컴포넌트를 자동으로 삽입하는 파이썬 스크립트를 작성했다. 이를 통해 블로그에서 사용하는 이미지가 더욱 빠르고 안정적으로 로드되도록 개선했다.",
};
